  ...
  static TypeId tid =
      TypeId ("P4SwitchApp")
          ...
          .AddAttribute ("GenerateProbability", "Probablity of packet generation",
            ...
            .AddAttribute ("TenantPolicy",
                   "Tenant-aware cache policy: 0=Baseline, 1=StaticPartitioning, 2=DynamicWeightedEviction",
                   IntegerValue (0), MakeIntegerAccessor (&P4SwitchApp::m_tenantPolicy),
                   MakeIntegerChecker<int32_t> ())
            .AddAttribute ("TenantCount", "Number of active tenants", UintegerValue (2),
                   MakeUintegerAccessor (&P4SwitchApp::m_tenantCount),
                   MakeUintegerChecker<uint32_t> (1))
            .AddAttribute ("PremiumTenantId", "Premium tenant id", UintegerValue (0),
                   MakeUintegerAccessor (&P4SwitchApp::m_premiumTenantId),
                   MakeUintegerChecker<uint32_t> ())
            .AddAttribute ("TenantKeyModulo",
                   "Tenant extraction key modulo (tenant = key % modulo % tenantCount)",
                   UintegerValue (2), MakeUintegerAccessor (&P4SwitchApp::m_tenantKeyModulo),
                   MakeUintegerChecker<uint32_t> (1))
            .AddAttribute (
              "PremiumProtectProbability",
              "Eviction protection probability when a non-premium entry competes with a premium entry",
              DoubleValue (0.8),
              MakeDoubleAccessor (&P4SwitchApp::m_premiumProtectProbability),
          ...
          .AddTraceSource ("CacheLookup",
                           "A packet triggered a cache lookup on the switch",
                           MakeTraceSourceAccessor (&P4SwitchApp::m_cacheLookup),
                           "ns3::Packet::SwitchIdTracedCallback")
                    ...
                    uint32_t podCount, unordered_map<uint32_t, uint32_t> *virtualToPhysical)
{
  ...
  m_cache.ConfigureTenantAware (m_tenantPolicy, m_tenantCount, m_premiumTenantId,
                                m_tenantKeyModulo, m_premiumProtectProbability);
                                ...
                                Ptr<Packet> packet, Ipv4Header &ipHeader)
{
  if (m_gwAddresses.count (physicalDestinationIp))
    {
      m_cacheLookup (packet, GetNode ()->GetId ());
