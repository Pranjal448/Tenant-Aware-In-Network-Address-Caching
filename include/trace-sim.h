...
private:
  ...
  void RecordCongState (const TcpSocketState::TcpCongState_t, const TcpSocketState::TcpCongState_t,
                        const uint32_t &);
  void CacheLookup (Ptr<const Packet> packet, uint32_t switchId);
  uint32_t GetTenantIdFromVirtualIp (uint32_t virtualIp) const;

  void CalculateDestinations ();
  void PopulateFlowToTenantMap ();
  ...
  unordered_map<uint32_t, uint64_t> m_switchToProcessedPackets, m_switchToCacheHits,
    ...
    unordered_map<uint32_t, uint64_t> m_tenantToCacheHits, m_tenantToCacheLookups, m_tenantToFlows,
      m_tenantToTotalFct;
    unordered_map<uint32_t, uint32_t> m_flowToTenant;
