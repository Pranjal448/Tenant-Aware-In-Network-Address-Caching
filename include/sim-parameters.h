#ifndef SIM_PARAMETERS_H
#define SIM_PARAMETERS_H

#include <boost/assign/list_of.hpp>
#include <map>
#include <string>
#include <vector>

using std::map;
using std::string;
using std::vector;

class SimulationParameters
{
public:
  enum Mode { Controller, SwitchV2P, LocalLearning, NoCache, GwCache, Direct, Bluebird, OnDemand };
  enum Topology { CLOS, FATTREE };
  enum TenantCachePolicy { Baseline = 0, StaticPartitioning = 1, DynamicWeightedEviction = 2 };

  SimulationParameters (string simMode, string networkTopology, size_t numOfPorts, size_t numOfCore,
                        size_t podWidth, vector<uint32_t> gatewayLeaves, bool randomRouting,
                        bool gatewayPerFlowLoadBalancing, bool udpMode,
                        string tenantPolicy, uint32_t tenantCount, uint32_t premiumTenantId,
                        uint32_t tenantKeyModulo, double premiumProtectProbability);

  enum Mode SimMode;
  enum Topology NetworkTopology;
  size_t NumOfPorts;
  size_t NumOfCore;
  size_t PodWidth;
  vector<uint32_t> GatewayLeaves;
  bool RandomRouting;
  bool GatewayPerFlowLoadBalancing;
  bool UdpMode;
  enum TenantCachePolicy TenantPolicy;
  uint32_t TenantCount;
  uint32_t PremiumTenantId;
  uint32_t TenantKeyModulo;
  double PremiumProtectProbability;

private:
  static const map<string, enum Mode> simulationModeMap;
  static const map<string, enum Topology> simulationTopologyMap;
  static const map<string, enum TenantCachePolicy> simulationTenantPolicyMap;
};

#endif /* SIM_PARAMETERS_H */