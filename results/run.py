...
def get_command_line(simMode, workload, config, output_file, topoScaling, gwScaling):
    ...
    return cli.format(
        ...
        config.get("tenantPolicy", "Baseline"),
        config.get("tenantCount", 2),
        config.get("premiumTenantId", 0),
        config.get("tenantKeyModulo", 2),
        config.get("premiumProtectProbability", 0.8),
        PLACEMENT[workload],
        TRACE[workload],
        output_file,
    )
...
def get_default_output_dir(workload, topoScaling, gwScaling, tenantEval=False):
    if tenantEval:
        dirName = f"{workload}_tenant_eval"
    elif topoScaling:
...
def run_workload(workload, gwScaling=False, topoScaling=False, outputDir=None, tenantEval=False, maxProc=None):
    ...
    if not outputDir:
        outputDir = get_default_output_dir(workload, topoScaling, gwScaling, tenantEval)

    effective_max_proc = maxProc if maxProc else MAX_PROC

    if tenantEval:
        PERCENTAGES = [x / 100 for x in [5, 10, 25, 50]]
    elif gwScaling or topoScaling:
    ...
    sim_modes = ["SwitchV2P"] if tenantEval else get_sim_modes()
    ...
    for p in PERCENTAGES:
        for mode in sim_modes:
            ...
            for config in configs:
                if tenantEval:
                    tenant_policies = ["Baseline", "StaticPartitioning", "DynamicWeightedEviction"]
                else:
                    tenant_policies = ["Baseline"]

                for tenant_policy in tenant_policies:
                    config = dict(config)
                    config["tenantPolicy"] = tenant_policy
                    config["tenantCount"] = 2
                    config["premiumTenantId"] = 0
                    config["tenantKeyModulo"] = 2
                    config["premiumProtectProbability"] = 0.8
                    ...
                    if len(proc_handles) >= effective_max_proc:
...
def main():
    ...
    parser.add_argument('--tenantEval', action='store_true',
                    help='Run a lightweight tenant-aware policy comparison (Baseline vs StaticPartitioning vs DynamicWeightedEviction)')
    parser.add_argument('--maxProc', type=int, default=6,
                    help='Maximum concurrent ns-3 processes (default: 6)')
    args = parser.parse_args()

    run_workload(args.workload, args.gw, args.topo, tenantEval=args.tenantEval, maxProc=args.maxProc)
