...
{
  PopulateFlowToTenantMap ();
  CalculateDestinations ();
}

void
TraceSimulation::PopulateFlowToTenantMap ()
{
  for (const auto &srcToFlows : m_containerToFlows)
    {
      for (const Flow &f : srcToFlows.second)
        {
          uint32_t dstVirtualIp = IpUtils::GetContainerVirtualAddress (f.dst).Get ();
          m_flowToTenant[f.flowId] = GetTenantIdFromVirtualIp (dstVirtualIp);
        }
    }
}
...
uint32_t
TraceSimulation::GetTenantIdFromVirtualIp (uint32_t virtualIp) const
{
  return (virtualIp % m_simParameters.TenantKeyModulo) % m_simParameters.TenantCount;
}
...
TraceSimulation::StopSimulation ()
{
  ...
  for (pair<uint32_t, FlowStats> fsPair : m_flowStats)
    {
      ...
      uint32_t tenantId = 0;
      if (m_flowToTenant.count (fsPair.first) > 0)
        {
          tenantId = m_flowToTenant.at (fsPair.first);
        }
      m_tenantToFlows[tenantId]++;
      m_tenantToTotalFct[tenantId] += fs.last.rxTime - fs.first.txTime;
    }
  ...
  unordered_map<uint32_t, double> tenantToHitRate;
  unordered_map<uint32_t, double> tenantToAvgFct;
  for (uint32_t tenantId = 0; tenantId < m_simParameters.TenantCount; ++tenantId)
    {
      uint64_t lookups = m_tenantToCacheLookups[tenantId];
      uint64_t hits = m_tenantToCacheHits[tenantId];
      tenantToHitRate[tenantId] = lookups == 0 ? 0.0 : hits / static_cast<double> (lookups);

      uint64_t flowCount = m_tenantToFlows[tenantId];
      tenantToAvgFct[tenantId] =
          flowCount == 0 ? 0.0 : m_tenantToTotalFct[tenantId] / static_cast<double> (flowCount);
    }

  json.add_child ("tenant_to_cache_hits", CreatePtree (m_tenantToCacheHits));
  json.add_child ("tenant_to_cache_lookups", CreatePtree (m_tenantToCacheLookups));
  json.add_child ("tenant_to_hit_rate", CreatePtree (tenantToHitRate));
  json.add_child ("tenant_to_avg_fct", CreatePtree (tenantToAvgFct));
...
TraceSimulation::CacheHit (Ptr<const Packet> packet, uint32_t switchId)
{
  ...
  if (packet->PeekPacketTag (flowIdTag) && m_flowToTenant.count (flowIdTag.GetFlowId ()) > 0)
    {
      m_tenantToCacheHits[m_flowToTenant.at (flowIdTag.GetFlowId ())]++;
    }
...
void
TraceSimulation::CacheLookup (Ptr<const Packet> packet, uint32_t)
{
  FlowIdTag flowIdTag;
  if (packet->PeekPacketTag (flowIdTag) && m_flowToTenant.count (flowIdTag.GetFlowId ()) > 0)
    {
      m_tenantToCacheLookups[m_flowToTenant.at (flowIdTag.GetFlowId ())]++;
    }
}
  ...
  for (uint32_t i = 0; i < m_switchApps.GetN (); ++i)
    {
      ...
      m_switchApps.Get (i)->TraceConnectWithoutContext (
          "CacheLookup", MakeCallback (&TraceSimulation::CacheLookup, this));
      m_switchApps.Get (i)->TraceConnectWithoutContext (
